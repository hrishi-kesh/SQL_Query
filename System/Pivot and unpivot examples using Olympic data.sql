...
--Examples of pivoting and unpivoting data. Uses a subset of the results from the Rio Olympics as a data source. For further explanation of the scripts, read the following blog post: https://blogs.oracle.com/sql/entry/how_to_convert_rows_to
Area
...
This pivots the results by medal. But the columns not listed in the pivot form an implicit group by. So this gives the medal total per athlete per event.
select * from olympic_medal_winners   
...
To overcome the problem in the previous statement, this selects just the columns you need in the subquery. But some events have multiple people who win the same medal - e.g. doubles tennis. This pivot counts rows in the table, not individual events.
select * from (   
...
You can have many functions in the pivot. Oracle generates a column for each function per value in the in clause. This finds the gold medal winning countries. For each it shows: - The number of different events these were won in - The number of different sports thy were won in - The names of the athlete or team who won each medal Finally it filters to only show those countries that won at least two gold medals.
select * from (   
...
a matrix, sports across the top countries down the side. There's an X for each sport that country has a row in the table for.
...
5 rows selected.
Statement 189
An example of how to build the pivot clause values dynamically. Note that when you do this the number of columns can change between runs. So the execute and fetch routine will be far more complex in a real world scenario!
declare   
  sql_stmt     clob;   
  pivot_clause clob;   
begin   
...
pivot (count(*) for sport in ('Archery' as "Archery",'Artistic Gymnastics' as "Artistic Gymnastics",'Athletics' as "Athletics",'Badminton' as "Badminton",'Beach Volleyball' as "Beach Volleyball",'Boxing' as "Boxing",'Canoe Slalom' as "Canoe Slalom",'Canoe Sprint' as "Canoe Sprint",'Cycling Road' as "Cycling Road",'Cycling Track' as "Cycling Track",'Diving' as "Diving",'Equestrian' as "Equestrian",'Fencing' as "Fencing",'Handball' as "Handball",'Hockey' as "Hockey",'Judo' as "Judo",'Modern Pentathlon' as "Modern Pentathlon",'Rhythmic Gymnastics' as "Rhythmic Gymnastics",'Rowing' as "Rowing",'Sailing' as "Sailing",'Swimming' as "Swimming",'Table Tennis' as "Table Tennis",'Taekwondo' as "Taekwondo",'Tennis' as "Tennis",'Trampoline Gymnastics' as "Trampoline Gymnastics",'Triathlon' as "Triathlon",'Volleyball' as "Volleyball",'Water Polo' as "Water Polo",'Weightlifting' as "Weightlifting",'Wrestling' as "Wrestling"))
Statement 190
The XML keyword dynamically builds the list of values to pivot. But you get the results in XML! Each "column" is an element in this document.
select * from (   
 select noc, sport   
 from   olympic_medal_winners   
)   
...
NOC	SPORT_XML
ARG	
<PivotSet><item><column name = "SPORT">Archery</column><column name = "MEDAL_WINNERS">1</column></item><item><column name = "SPORT">Artistic Gymnastics</column><column name = "MEDAL_WINNERS">1</column></item><item><column name = "SPORT">Athletics</column><column name = "MEDAL_WINNERS">1</column></item></PivotSet>
Statement 191
...
NOC	SPORT_XML
ARG	
<PivotSet><item><column name = "SPORT">Archery</column><column name = "MEDAL_WINNERS">0</column></item><item><column name = "SPORT">Artistic Gymnastics</column><column name = "MEDAL_WINNERS">0</column></item><item><column name = "SPORT">Athletics</column><column name = "MEDAL_WINNERS">0</column></item></PivotSet>
Statement 192
...
BRA	GOLD	2	Brazil,Cerutti,Oscar Schmidt
FRA	GOLD	2	Azou,Houin,RINER Teddy
...
Athletics	0	0	0	0	1	0
Artistic Gym	2	0	0	0	0	0
Boxing	0	1	0	0	0	0
Sailing	0	0	0	0	0	2
Track Cycling	0	0	1	0	0	0
